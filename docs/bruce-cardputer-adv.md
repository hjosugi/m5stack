...
ただしBruceは、Wi-Fi、BLE、USB HID、赤外線、RF等のセキュリティ試験機能を含む第三者製ファームウェアです。所有または明示的な許可のある機器・電波環境だけで使用し、工場ファームウェアの復元経路を用意してから導入を判断します。

2026-08-01、対象をCardputer-Adv一台へ固定し、M5Launcher 2.8.0からBruce 1.16をinstallしました。配布物のSHA-256、Launcher側の転送完了、再起動後の`Bruce v1.16`と機種応答まで確認済みです。Wi-Fiや外付けRF moduleは使わず、factory復元用の公式UserDemo ADV-V0.3を先に確保しました。詳しい実測値は[作業記録](work-log.md)にあります。
